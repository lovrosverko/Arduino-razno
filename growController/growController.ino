...
bool  led01, relej, sendTemp, sendVlaga;
...
int citajVlagu() {
  int sensorValue = analogRead(senzorVlage);
  Serial.print("Vlaga: "); Serial.println(sensorValue);
  return sensorValue;
}
...
void sendMsg() {
  ...
  if (sendTemp) {
    ...
    if (temperatura > 100) {
      ...
      sprintf(buff_topic, "smartGrow/node%02d/temp", nodeId);
      char tempIspis[2];
  ...
  if (sendVlaga) {
    int vlaga = citajVlagu();
    delay(100);
    if (vlaga > 1000) {
      return;
    }
    else {
      sprintf(buff_topic, "smartGrow/node%02d/vlaga", nodeId);
      char tempIspis[2];
